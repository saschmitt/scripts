...
# Top left
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$cx:$cy:0:0 -keyint_min 1 "00"$outputFile)
# Top middle
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$ix:$cy:$cx:0 -keyint_min 1 "01"$outputFile)
# Top right
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$(( $x - ( $cx + $ix ) )):$cy:$(( $cx + $ix )):0 -keyint_min 1 "02"$outputFile)

# Middle left
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$cx:$iy:0:$cy -keyint_min 1 "10"$outputFile)
# Middle middle
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$ix:$iy:$cx:$cy -keyint_min 1 "11"$outputFile)
# Middle right
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$(( $x - ( $cx + $ix ) )):$iy:$(( $cx + $ix )):$cy -keyint_min 1 "12"$outputFile)

# Bottom left
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$cx:$(( $y - ( $cy + $iy ) )):0:$(( $cy + $iy )) -keyint_min 1 "20"$outputFile)
# Bottom middle
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$ix:$(( $y - ( $cy + $iy ) )):$cx:$(( $cy + $iy )) -keyint_min 1 "21"$outputFile)
# Bottom right
echo $(ffmpeg -i $inputFile -strict experimental -r 25 -vf crop=$(( $x - ( $cx + $ix ) )):$(( $y - ( $cy + $iy ) )):$(( $cx + $ix )):$(( $cy + $iy )) -keyint_min 1 "22"$outputFile)
